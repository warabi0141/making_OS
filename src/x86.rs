...
pub unsafe fn write_cs(cs: u16) {
    ...
    asm!(
        ...
        "push cx", // Construct a far poiner on the stack
        "push rax",
        "ljmp [rsp]",
        "2:",
...
struct InterruptInfo {
    ...
    fpu_context: FPUContext, // used by FXSAVE / FXSTOR
    _dummy: u64,
...
///     push rcx // Save rcs first to reuse
///     mov rcx, N // INT#
...
impl GdtWrapper {
    pub fn load(&self) {
        ...
        // SAFETY: This is safe scice it is loading a valid GDT just constructed is the above
        unsafe {
            asm!(
                "lgdt [rcx]",
                in("rcx") &params,
