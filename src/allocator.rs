...
// core::cell::RefCell : 借用チェックをコンパイル時じゃなくて実行時に行う
...
#[allow(clippy::assertions_on_contants)]
...
impl FirstFitAllocator {
        ...
        loop {
            match header {
                Some(e) => match e.provide(layout.size(), layout.align()) {
                    // Someならメモリの確保ができた、Noneなら確保できなかった
                    Some(p) => break p,
                    None => {
                        // eの中身を次の空き領域へのポインタに変える
                        header = e.next_header.borrow_mut();
                        continue;
                    }
                },
                None => {
                    // 全探索した結果メモリ割り当てができずに諦めた
                    break null_mut::<u8>();
                }
            }
        }
    ...
    fn add_free_from_description(&self, desc: &EfiMemoryDescriptor) {
        ...
        // since all the regions written in memory map are not contiguous
